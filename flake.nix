{
  description = "CocoBrowser development environment and quick-start commands";
      ...
      let
        ...
        releaseAppImage =
          if system == "x86_64-linux" then
            pkgs.fetchurl {
              url = "https://github.com/zhom/cocobrowser/releases/download/v0.28.1/Coco_0.28.1_amd64.AppImage";
              hash = "sha256-ItjyK206mpwMXzTNnSHHM8L2R8EjvEP3mrKnz6ze4oI=";
            }
          else if system == "aarch64-linux" then
            pkgs.fetchurl {
              url = "https://github.com/zhom/cocobrowser/releases/download/v0.28.1/Coco_0.28.1_aarch64.AppImage";
          ...
          if releaseAppImage != null then
            pkgs.stdenvNoCC.mkDerivation {
              ...
              installPhase = ''
                ...
                cp "$src" ./coco.AppImage
                chmod +x ./coco.AppImage
                ./coco.AppImage --appimage-extract >/dev/null
          ...
          if releaseUnpacked != null then
            pkgs.writeShellApplication {
              ...
              text = ''
                ...
                if [ -x "${releaseWrapped}/bin/coco-release" ]; then
                  if "${releaseWrapped}/bin/coco-release" "$@"; then
                    exit 0
                  fi
      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "CocoBrowser dev shell ready."
            echo "Quick start:"
            echo "  nix run .#setup"
        ...
        apps.deps = mkApp "coco-deps" ''
          set -euo pipefail
          pnpm install
        '';

        apps.dev = mkApp "coco-dev" ''
          set -euo pipefail
          pnpm dev
        '';

        apps."tauri-dev" = mkApp "coco-tauri-dev" ''
          set -euo pipefail
          pnpm tauri dev
        '';

        apps."full-dev" = mkApp "coco-full-dev" ''
          set -euo pipefail
          chmod +x ./scripts/dev.sh
          ./scripts/dev.sh
        '';

        apps.build = mkApp "coco-build" ''
          set -euo pipefail
          pnpm build
          (cd src-tauri && cargo build)
        '';

        apps.start = mkApp "coco-start" ''
          set -euo pipefail
          pnpm start
        '';

        apps.test = mkApp "coco-test" ''
          set -euo pipefail
          pnpm format && pnpm lint && pnpm test
        '';

        apps.setup = mkApp "coco-setup" ''
          set -euo pipefail

          if [ ! -f "package.json" ]; then
            echo "package.json not found. Run this from the cocobrowser repo root."
            exit 1
          fi
        ...
        apps."release-start" = {
          type = "app";
          program = "${releaseLauncher}/bin/coco-release-start";
        };
