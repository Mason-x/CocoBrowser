 ...
 * 2. Builds and starts coco-sync server
...
async function buildCocoSync() {
  log("Building coco-sync...");
  ...
  const syncDir = path.join(ROOT_DIR, "coco-sync");
  ...
  if (!existsSync(path.join(syncDir, "dist", "main.js"))) {
    throw new Error("coco-sync build did not produce dist/main.js");
  }
  log("coco-sync built");
}

async function startCocoSync() {
  log(`Starting coco-sync on port ${SYNC_PORT}...`);

  const proc = spawn("node", ["dist/main.js"], {
    cwd: path.join(ROOT_DIR, "coco-sync"),
    env: {
      ...process.env,
  ...
  proc.stdout.on("data", (data) => {
    if (process.env.VERBOSE) {
      console.log(`[coco-sync] ${data.toString().trim()}`);
    }
  });

  proc.stderr.on("data", (data) => {
    if (process.env.VERBOSE) {
      console.error(`[coco-sync] ${data.toString().trim()}`);
    }
  });

  proc.on("error", (err) => {
    error(`coco-sync error: ${err.message}`);
  });

  await waitForHealth(`http://localhost:${SYNC_PORT}/health`, 30000);
  log("coco-sync is ready");
