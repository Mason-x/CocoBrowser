...
export type BackendErrorCode =
  ...
  | "COOKIE_EXPORT_PASSWORD_TOO_SHORT"
  ...
  | "CLOUD_PROXY_NOT_SUPPORTED"
  | "PROXY_EXIT_CHECK_FAILED"
...
export function translateBackendError(t: TFunction, err: unknown): string {
  ...
  switch (parsed.code) {
    ...
    case "COOKIE_EXPORT_PASSWORD_TOO_SHORT":
      return t("backendErrors.cookieExportPasswordTooShort", {
        min: parsed.params?.min ?? "12",
      });
    case "COOKIE_EXPORT_PASSWORD_REQUIRED":
      return t("backendErrors.cookieExportPasswordRequired");
    case "COOKIE_EXPORT_DECRYPT_FAILED":
    ...
    case "IMPORT_SOURCE_UNSUPPORTED":
      return t("backendErrors.importSourceUnsupported", {
        browser: parsed.params?.browser ?? "",
      });
    case "IMPORT_SOURCE_INVALID":
      return t("backendErrors.importSourceInvalid");
    case "IMPORT_FAILED":
      return t("backendErrors.importFailed", {
    ...
    case "CLOUD_PROXY_NOT_SUPPORTED":
      return t("backendErrors.cloudProxyNotSupported");
    case "PROXY_EXIT_CHECK_FAILED":
      return t("backendErrors.proxyExitCheckFailed", {
        reason: parsed.params?.reason ?? "",
      });
    case "GEO_TIMEZONE_MISMATCH":
      return t("backendErrors.geoTimezoneMismatch", {
        reason: parsed.params?.reason ?? "",
