...
interface TableMeta {
  ...
  // Profile groups (for the Group column)
...
/**
 * Group picker rendered inline in the table. Assigning writes through
 * `assign_profiles_to_group`; the backend then emits `profiles-changed`,
 * which reloads the list, so there is no local override to keep in sync.
 */
const GroupCell = React.memo<{
  profile: BrowserProfile;
  isDisabled: boolean;
  groups: ProfileGroup[];
  t: (key: string) => string;
}>(({ profile, isDisabled, groups, t }) => {
  ...
  return (
    <Select
      value={profile.group_id ?? "__none__"}
      disabled={isDisabled}
      onValueChange={(value) => void onChange(value)}
    >
      <SelectTrigger
        size="sm"
        className="h-6 w-full border-0 bg-transparent px-2 text-xs shadow-none focus:ring-0"
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="__none__">{t("profileInfo.values.none")}</SelectItem>
        {groups.map((group) => (
...
export function ProfilesDataTable({
  ...
  const selectedInfoDialogProfile = isInfoDialogControlled
    ? (infoDialogProfile ?? null)
    : internalInfoDialogProfile;
  // The selection above is a snapshot taken when the dialog opened. Re-resolve
  // it against the live list so edits made inside the dialog (name, group,
  // tags, note) render immediately instead of only after reopening it.
  const profileForInfoDialog = React.useMemo(() => {
    if (!selectedInfoDialogProfile) return null;
    return (
      profiles.find((p) => p.id === selectedInfoDialogProfile.id) ??
      selectedInfoDialogProfile
    );
  }, [profiles, selectedInfoDialogProfile]);
  ...
  // Profile groups for the Group column. Refreshed when the backend emits
  // 'groups-changed' (create/rename/delete).
  const [profileGroups, setProfileGroups] = React.useState<ProfileGroup[]>([]);
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await invoke<ProfileGroup[]>("get_profile_groups");
        if (mounted) setProfileGroups(data);
      } catch (e) {
  ...
  const columns: ColumnDef<BrowserProfile>[] = React.useMemo(
    () => [
      ...
      },
      {
        id: "group",
        size: 110,
        accessorFn: (profile) => profile.group_id ?? "",
        header: ({ table }) => {
          const meta = table.options.meta as TableMeta;
