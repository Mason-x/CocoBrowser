...
export function IntegrationsDialog({
  ...
  const handleSecurityFlag = async (
    key: "allow_headless_automation" | "allow_js_evaluate",
    enabled: boolean,
  ) => {
    ...
    try {
      ...
      showErrorToast(t("integrations.securitySaveFailed"), {
        description:
    ...
    >
     ...
     http://127.0.0.1:${apiServerPort ?? settings.api_port}/v1/profiles`}
            ...
            >
              ...
              <div className="flex flex-col gap-4 rounded-md border bg-card p-4">
                <div className="flex items-start gap-3">
                  <LuShieldCheck className="mt-0.5 size-5 text-muted-foreground" />
                  <div className="flex flex-col gap-1">
                    <Label className="text-sm font-medium">
                      {t("integrations.automationSecurityTitle")}
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      {t("integrations.automationSecurityDescription")}
                    </p>
                  </div>
                </div>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex flex-col gap-1">
                    <Label className="text-sm">
                      {t("integrations.allowHeadlessLabel")}
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      {t("integrations.allowHeadlessDescription")}
                    </p>
                  </div>
                  <AnimatedSwitch
                    checked={settings.allow_headless_automation}
                    onCheckedChange={(checked) =>
                      void handleSecurityFlag(
                        "allow_headless_automation",
                        checked,
                      )
                    }
                  />
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex flex-col gap-1">
                    <Label className="text-sm">
                      {t("integrations.allowEvaluateLabel")}
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      {t("integrations.allowEvaluateDescription")}
                    </p>
                  </div>
                  <AnimatedSwitch
                    checked={settings.allow_js_evaluate}
                    onCheckedChange={(checked) =>
                      void handleSecurityFlag("allow_js_evaluate", checked)
                    }
                  />
                </div>
              </div>
