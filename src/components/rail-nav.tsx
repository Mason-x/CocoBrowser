...
export function RailNav({ currentPage, onNavigate }: RailNavProps) {
  ...
  return (
    <nav
      className={cn(
        "relative flex shrink-0 flex-col gap-1 border-r border-border bg-background py-2 transition-[width] duration-200 ease-in-out select-none",
    ...
    >
      {!isHidden ? (
        <button
          ...
          className={cn(
            "grid size-7 shrink-0 cursor-pointer place-items-center rounded-md bg-transparent text-foreground select-none",
            expanded && "mx-auto",
          )}
          onClick={handleClick}
          onPointerDown={() => {
        ...
        <div className={cn("size-7 shrink-0", expanded && "mx-auto")} />
      )}
