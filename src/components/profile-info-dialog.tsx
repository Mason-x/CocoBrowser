...
/**
 * Shows the total bytes routed through Coco's local proxy worker for this
 * profile. Only counts traffic flowing through the coco-proxy binary — not
        ...
        <div className="scroll-fade min-w-0 flex-1 overflow-y-auto p-4">
          {section === "overview" && (
            <div className="flex flex-col gap-3">
              ...
              <ProfileBasicsEditor profile={profile} isRunning={isRunning} />
...
function NetworkSectionInline({
  ...
  const [isSaving, setIsSaving] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const [isEditingProxy, setIsEditingProxy] = React.useState(false);
  ...
  const selectedProxy = proxyId
    ? (storedProxies.find((proxy) => proxy.id === proxyId) ?? null)
    : null;
  ...
  return (
    <>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={
              ...
              isSaving ||
              !selectedProxy ||
              selectedProxy.is_cloud_managed ||
              selectedProxy.is_cloud_derived
            }
        ...
        {selectedProxy && (
          <p className="pl-14 font-mono text-[10px] text-muted-foreground">
            {selectedProxy.proxy_settings.proxy_type.toUpperCase()} ·{" "}
            {selectedProxy.proxy_settings.host}:
            {selectedProxy.proxy_settings.port}
          </p>
        )}
...
const PERSONA_CORE_OPTIONS = [2, 4, 6, 8, 12, 16, 24, 32] as const;
const PERSONA_SPOOFING_SURFACES = [
  "canvas",
  "audio",
  "fonts",
  "client_rects",
...
function normalizeLocalPersona(
  ...
  return {
    ...persona,
    acceptLanguages: persona.acceptLanguages?.length
      ? persona.acceptLanguages
      : [persona.language],
    spoofingDisabled: persona.spoofingDisabled ?? [],
  };
}

function FingerprintChromiumPersonaInline({
  profile,
  isDisabled,
  isRunning,
  onSaved,
  ...
  const [persona, setPersona] = React.useState<FingerprintPersona | null>(() =>
    profile.persona ? normalizeLocalPersona(profile.persona) : null,
  );
  const [initialPersona, setInitialPersona] =
    React.useState<FingerprintPersona | null>(() =>
      profile.persona ? normalizeLocalPersona(profile.persona) : null,
    );
  const [isSaving, setIsSaving] = React.useState(false);
  ...
  React.useEffect(() => {
    const next = profile.persona
      ? normalizeLocalPersona(profile.persona)
      : null;
    setPersona(next);
    setInitialPersona(next);
    setError(null);
  }, [profile.persona]);

  const updatePersona = <K extends keyof FingerprintPersona>(
    key: K,
    value: FingerprintPersona[K],
  ) => {
    setPersona((current) => (current ? { ...current, [key]: value } : current));
    setError(null);
  };
  ...
  const savePersona = async () => {
    ...
    try {
      ...
      const next = updated.persona
        ? normalizeLocalPersona(updated.persona)
        : persona;
      setPersona(next);
      setInitialPersona(next);
      ...
      setError(translateBackendError(t as never, saveError));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const regeneratePersona = async () => {
    if (isDisabled || isRunning || busy) return;
    if (!window.confirm(t("identity.regenerateConfirm"))) return;
    ...
    try {
      ...
      if (updated.persona) {
        const next = normalizeLocalPersona(updated.persona);
        setPersona(next);
        setInitialPersona(next);
      }
      showSuccessToast(t("identity.regenerated"));
    } catch (regenerateError) {
      setError(translateBackendError(t as never, regenerateError));
    } finally {
      setIsRegenerating(false);
    }
  };

  const matchPersona = async () => {
    if (isDisabled || isRunning || busy) return;
    setIsMatching(true);
    setError(null);
    try {
      ...
      setError(translateBackendError(t as never, matchError));
    } finally {
      setIsMatching(false);
    }
  };
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        <Button
          ...
          size="sm"
          className="w-fit"
          disabled={isDisabled || isRunning || isRegenerating}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <InfoCard label={t("identity.seed")} value={String(persona.seed)} />
        <InfoCard
          label={t("identity.schemaVersion")}
          value={String(persona.schemaVersion)}
        />
        <InfoCard
          label={t("identity.kernel")}
          value={`${profile.browser} ${profile.version}`}
        />
        <InfoCard
          label={t("identity.platform")}
          value={`${persona.platform} ${persona.platformVersion ?? ""}`.trim()}
        />
        <InfoCard
          label={t("identity.brandVersion")}
          value={`${persona.brand} ${persona.brandVersion}`}
        />
        <InfoCard
          label={t("identity.capabilityRevision")}
          value={persona.capabilityRevision}
        />
      </div>

      <div className="grid gap-3 sm:grid-cols-2">
        <div className="grid gap-1.5">
          <Label htmlFor="inline-persona-language">
            {t("identity.language")}
          </Label>
          <Input
            id="inline-persona-language"
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="inline-persona-timezone">
            {t("identity.timezone")}
          </Label>
          <Input
            id="inline-persona-timezone"
            value={persona.timezone}
            disabled={readOnly}
            onChange={(event) => updatePersona("timezone", event.target.value)}
          />
        </div>
        <div className="grid gap-1.5 sm:col-span-2">
          <Label htmlFor="inline-persona-languages">
            {t("identity.acceptLanguages")}
          </Label>
          <Input
            id="inline-persona-languages"
        ...
        <div className="grid gap-1.5">
          ...
          <Select
            ...
            disabled={readOnly}
            onValueChange={(value) =>
              updatePersona("hardwareConcurrency", Number(value))
            }
          >
            <SelectTrigger>
              <SelectValue />
            ...
            <SelectContent>
              {PERSONA_CORE_OPTIONS.map((cores) => (
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="inline-persona-width">
            {t("identity.windowWidth")}
          </Label>
          <Input
            id="inline-persona-width"
            type="number"
            min={800}
            max={7680}
            ...
            disabled={readOnly}
            onChange={(event) =>
              updatePersona("windowWidth", Number(event.target.value))
            }
          />
        </div>
        <div className="grid gap-1.5">
          <Label htmlFor="inline-persona-height">
            {t("identity.windowHeight")}
          </Label>
          <Input
            id="inline-persona-height"
            type="number"
            min={600}
            max={4320}
            ...
            disabled={readOnly}
            onChange={(event) =>
              updatePersona("windowHeight", Number(event.target.value))
            }
          />
        </div>
      </div>
      ...
      {persona.proxyGeoSignature && (
        <div className="grid gap-1.5">
          <Label htmlFor="inline-persona-geo-signature">
            {t("identity.geoStamp")}
          </Label>
          <Input
            id="inline-persona-geo-signature"
            className="font-mono text-xs"
            value={persona.proxyGeoSignature}
            disabled
          />
        </div>
      )}
...
function FingerprintSectionInline({
  ...
  if (profile.browser === "fingerprint-chromium") {
    return (
      <FingerprintChromiumPersonaInline
        profile={profile}
        isDisabled={isDisabled}
        isRunning={isRunning}
        onSaved={onSaved}
