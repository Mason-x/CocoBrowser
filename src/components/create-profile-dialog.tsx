...
export function CreateProfileDialog({
  ...
  // The local fingerprint kernel is the primary profile type, so the dialog
  ...
  const [kernelManifest, setKernelManifest] = useState<KernelManifest | null>(
    null,
  );
  ...
  const getCreatableVersion = useCallback(
    (browserType?: string) => {
      if (browserType === "fingerprint-chromium") {
        ...
        return installed
          ? { version: installed.version, releaseType: "stable" as const }
          : null;
      }
  ...
  const handleCreate = async () => {
    ...
    try {
      if (activeTab === "anti-detect") {
        ...
        // Use the latest available local browser version.
  ...
  const isBrowserVersionAvailable = useCallback(
        ...
        return Boolean(
          bestVersion &&
            installedKernels.some(
              (kernel) =>
                kernel.id === browserStr &&
                kernel.version === bestVersion.version,
            ),
        );
      }
      const bestVersion = getBestAvailableVersion(browserStr);
    ...
    [isVersionDownloaded, getBestAvailableVersion, installedKernels],
  );
