...
/// Setup function to ensure coco-proxy binary exists and cleanup stale proxies
async fn setup_test() -> Result<std::path::PathBuf, Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Build coco-proxy binary if it doesn't exist
  ...
  if !proxy_binary.exists() {
    println!("Building coco-proxy binary for integration tests...");
...
async fn test_traffic_tracking() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let traffic_stats_dir = cocobrowser_lib::app_dirs::cache_dir().join("traffic_stats");
...
async fn test_bypass_rules_http_direct() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Start coco-proxy with upstream + bypass rules for "127.0.0.1"
  ...
  println!("Coco-proxy started on port {local_port} with bypass rules for 127.0.0.1");
...
async fn test_bypass_rules_in_config() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let proxies_dir = cocobrowser_lib::app_dirs::proxy_workers_dir();
...
/// Test that a SOCKS5 upstream proxy works end-to-end through coco-proxy.
/// Starts a mock SOCKS5 server, a mock HTTP target server,
/// then routes requests through coco-proxy -> SOCKS5 -> target.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_socks5_upstream(
  ...
  // Test 1: HTTP request through coco-proxy -> SOCKS5 -> target
...
/// Verifies the full chain: client → coco-proxy → Shadowsocks → internet.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_shadowsocks_upstream(
  ...
  let ss_container = "coco-ss-test";
  let ss_port = 18388u16;
  let ss_password = "coco-test-password";
  ...
  // Start coco-proxy with Shadowsocks upstream
  let output = TestUtils::execute_command(
    &binary_path,
    &[
      "proxy",
  ...
  // Test: HTTP request through coco-proxy → Shadowsocks → example.com
  let mut stream = TcpStream::connect(("127.0.0.1", local_port)).await?;
  let request =
