...
async fn test_wireguard_tunnel_init() {
  ...
  use cocobrowser_lib::vpn::{VpnTunnel, WireGuardTunnel};
...
async fn test_tunnel_manager() {
  use cocobrowser_lib::vpn::{TunnelManager, VpnStatus, VpnTunnel};
  ...
  impl VpnTunnel for MockTunnel {
    async fn connect(&mut self) -> Result<(), cocobrowser_lib::vpn::VpnError> {
      self.connected = true;
      Ok(())
    }

    async fn disconnect(&mut self) -> Result<(), cocobrowser_lib::vpn::VpnError> {
...
impl TestEnvGuard {
  fn new() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let root = TEST_RUNTIME_ROOT
      .get_or_init(|| {
        std::env::temp_dir().join(format!("cocobrowser-vpn-e2e-{}", std::process::id()))
      })
      .clone();
    ...
    let previous_data_dir = std::env::var("COCOBROWSER_DATA_DIR").ok();
    let previous_cache_dir = std::env::var("COCOBROWSER_CACHE_DIR").ok();

    std::env::set_var("COCOBROWSER_DATA_DIR", &data_dir);
    std::env::set_var("COCOBROWSER_CACHE_DIR", &cache_dir);
...
impl Drop for TestEnvGuard {
  fn drop(&mut self) {
    if let Some(value) = &self.previous_data_dir {
      std::env::set_var("COCOBROWSER_DATA_DIR", value);
    } else {
      std::env::remove_var("COCOBROWSER_DATA_DIR");
    }

    if let Some(value) = &self.previous_cache_dir {
      std::env::set_var("COCOBROWSER_CACHE_DIR", value);
    } else {
      std::env::remove_var("COCOBROWSER_CACHE_DIR");
    }
  }
}
...
async fn ensure_coco_proxy_binary() -> Result<PathBuf, Box<dyn std::error::Error + Send + Sync>> {
...
async fn cleanup_runtime() {
  let _ = cocobrowser_lib::proxy_runner::stop_all_proxy_processes().await;
  let _ = cocobrowser_lib::vpn_worker_runner::stop_all_vpn_workers().await;
...
async fn run_proxy_feature_suite(
  ...
  let vpn_worker = cocobrowser_lib::vpn_worker_runner::start_vpn_worker(vpn_id)
    .await
  ...
  assert!(
    ...
    "HTTP traffic through coco-proxy+VPN tunnel should succeed, got: {}",
    &http_response[..http_response.len().min(300)]
  );

  let stats_file = cocobrowser_lib::app_dirs::cache_dir()
    .join("traffic_stats")
  ...
  assert!(
    ...
    "Bypass rules should still work when coco-proxy is chained to a VPN worker"
  );
  stop_proxy(binary_path, &bypass_proxy.id).await?;
  bypass_server.abort();

  cocobrowser_lib::vpn_worker_runner::stop_vpn_worker(&vpn_worker.id)
    .await
    .map_err(|error| error.to_string())?;
  Ok(())
}
...
#[tokio::test]
#[serial]
async fn test_wireguard_traffic_flows_through_coco_proxy(
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let binary_path = ensure_coco_proxy_binary().await?;
  ...
  {
    let storage = cocobrowser_lib::vpn::VPN_STORAGE.lock().unwrap();
