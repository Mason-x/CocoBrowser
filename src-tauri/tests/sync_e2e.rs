use cocobrowser_lib::sync::types::*;
use reqwest::Client;
use serde_json::json;
use std::env;
use std::fs;
    ...
    Ok(response) => {
      if !response.status().is_success() {
        panic!(
          ...
          Server URL: {}\n\
          Please ensure:\n\
          1. MinIO is running (docker compose up -d in coco-sync/)\n\
          2. coco-sync server is running (cd coco-sync && pnpm start:dev)\n\
...
async fn test_profile_bypass_rules_sync() {
  ...
  assert_eq!(metadata["browser"], "fingerprint-chromium");
...
async fn test_encrypted_profile_sync() {
  use cocobrowser_lib::sync::encryption::{
