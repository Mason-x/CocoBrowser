...
impl WayfernManager {
  ...
  /// geolocation fetch. Loopback socks URLs are the app's own coco-proxy
  ...
  pub async fn generate_fingerprint_config(
    ...
    let (fingerprint, geolocation_applied) = match get_result {
      Ok(result) => {
        ...
        // Route the geolocation lookup through a temporary local coco-proxy
  ...
  pub async fn launch_wayfern(
    ...
    // Local-first build: never request or inject Wayfern cloud tokens.
