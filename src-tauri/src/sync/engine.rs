...
impl SyncResumeState {
  fn path(profile_dir: &Path) -> std::path::PathBuf {
    profile_dir.join(".coco-sync").join("resume-state.json")
  }
...
/// Check if sync is configured. Self-hosted is the only supported mode.
...
impl SyncEngine {
  ...
  /// Key prefix for remote objects. Teams were a hosted-account feature, so
  /// every object now lives at the personal (unprefixed) path.
  async fn get_team_key_prefix(_profile: &BrowserProfile) -> String {
    String::new()
  }

  /// Sync is always self-hosted in this build.
  ...
  pub async fn check_for_missing_synced_profiles(
    ...
    // Delete local synced profiles that have a remote tombstone (deleted on another device)
    {
      ...
      // Teams were a hosted-account feature; only personal paths exist now.
