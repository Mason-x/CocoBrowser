...
struct ImportCookiesRequest {
  ...
  content: String,
  /// Required only for a Coco encrypted `.cocookies` bundle.
...
impl ApiServer {
  ...
  async fn start(
    ...
    let v1_routes = v1_routes
      ...
      // Host/Origin rebinding guard (before auth so probes fail closed early).
      .layer(middleware::from_fn(loopback_host_origin_middleware));
    ...
    let app = Router::new()
      ...
      // Local-first: no browser CORS. Automation clients do not need CORS.
      .layer(CorsLayer::new())
      .with_state(state);
...
/// Reject DNS-rebinding Host headers and browser Origin requests.
/// Only loopback hosts are accepted. Browser Origin is denied by default
...
/// Hosts/origins allowed from local automation tooling only.
fn is_allowed_origin(origin: &str) -> bool {
  // Explicit empty = missing. Reject http(s) browser pages on other hosts.
  if origin.is_empty() || origin == "null" {
    return true;
  }
  // Allow tauri custom schemes if present.
  if origin.starts_with("tauri://") || origin.starts_with("http://tauri.localhost") {
    return true;
  }
  // Loopback http origins with any port.
  origin.starts_with("http://127.0.0.1")
    || origin.starts_with("http://localhost")
    || origin.starts_with("http://[::1]")
}

async fn loopback_host_origin_middleware(
  headers: HeaderMap,
  ...
  if let Some(host) = headers.get("host").and_then(|h| h.to_str().ok()) {
    if !is_loopback_host(host) {
      log::warn!("[api] Rejected {path}: non-loopback Host={host}");
  ...
  if let Some(origin) = headers.get("origin").and_then(|h| h.to_str().ok()) {
    if !is_allowed_origin(origin) {
      log::warn!("[api] Rejected {path}: disallowed Origin={origin}");
      return Err(StatusCode::FORBIDDEN);
    }
  }

  // Content-Length hard cap (16 MiB) for management payloads.
  if let Some(cl) = headers.get("content-length").and_then(|h| h.to_str().ok()) {
    if let Ok(n) = cl.parse::<u64>() {
      if n > 16 * 1024 * 1024 {
        log::warn!("[api] Rejected {path}: body too large ({n} bytes)");
...
async fn rate_limit_middleware(
  ...
  if request.uri().path().starts_with("/v1/")
    && !crate::local_rate_limit::API_RATE_LIMITER.try_acquire()
  {
...
/// - A stable local Persona is generated at creation.
#[utoipa::path(
  post,
  path = "/v1/profiles",
...
async fn create_profile(
  ...
  // Reject a dead or unreachable self-hosted proxy/VPN before creation.
  if let Err(err) =
...
async fn run_profile(
  ...
  // Local-first: automation is free. Headless remains opt-in via settings.
  let headless = request.headless.unwrap_or(false);
  if headless
    && !crate::settings_manager::SettingsManager::instance()
      .load_settings()
      .map(|s| s.allow_headless_automation)
      .unwrap_or(false)
  {
    log::warn!("[api] rejected headless request because allow_headless_automation is false");
...
async fn kill_profile(
  ...
  // Local automation is available without a cloud subscription.
  let profile_manager = ProfileManager::instance();
  let profiles = profile_manager
    .list_profiles()
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
// API Handler - Batch stop local profiles.
#[utoipa::path(
  post,
...
mod tests {
  ...
  fn loopback_host_accepts_localhost_variants() {
    ...
    assert!(is_loopback_host("127.0.0.1:10108"));
    assert!(is_loopback_host("localhost:51080"));
    assert!(is_loopback_host("[::1]:8080"));
    assert!(!is_loopback_host("evil.example.com"));
    assert!(!is_loopback_host("192.168.1.1"));
    assert!(!is_loopback_host("0.0.0.0:10108"));
  }

  #[test]
  fn origin_policy_rejects_foreign_browser_origins() {
    ...
    assert!(!is_allowed_origin("http://192.168.0.5"));
  }
