...
fn unsuffixed_binary_name(base_name: &str) -> String {
  ...
  {
    match base_name {
      "coco-proxy" => "coco-proxy.exe".to_string(),
      _ => String::new(),
    }
  }
...
pub async fn start_proxy_process_with_profile(
  ...
  let exe = find_sidecar_executable("coco-proxy")?;
  ...
  #[cfg(unix)]
  {
    ...
    cmd.arg("--id");
    cmd.arg(&id);
    cmd.env_remove("COCO_PROXY_USERNAME");
    cmd.env_remove("COCO_PROXY_PASSWORD");

    cmd.stdin(Stdio::null());
    cmd.stdout(Stdio::null());

    // Always log to file for diagnostics (both debug and release builds)
    let log_path = std::env::temp_dir().join(format!("coco-proxy-{}.log", id));
