...
impl CookieManager {
  const ENCRYPTED_EXPORT_HEADER: &'static str = "COCO_COOKIE_EXPORT_V1";
  /// Header written by Donut Browser, this fork's upstream. Still accepted on
  /// import so bundles exported before the rename stay readable; never written.
  const LEGACY_EXPORT_HEADER: &'static str = "DONUT_COOKIE_EXPORT_V1";

  /// Strip whichever encrypted-export header the bundle carries.
  fn strip_export_header(content: &str) -> Option<&str> {
    content
      .strip_prefix(Self::ENCRYPTED_EXPORT_HEADER)
      .or_else(|| content.strip_prefix(Self::LEGACY_EXPORT_HEADER))
  }

  /// True when the content is an encrypted export in either header format.
  fn has_export_header(content: &str) -> bool {
    content.starts_with(Self::ENCRYPTED_EXPORT_HEADER)
      || content.starts_with(Self::LEGACY_EXPORT_HEADER)
  }
  const ENCRYPTED_EXPORT_AAD: &'static str = "coco-cookie-export:v1";
  const ENCRYPTED_EXPORT_KDF: &'static str = "argon2id-v19-m19456-t2-p1";
  const MAX_COOKIE_IMPORT_BYTES: usize = 50 * 1024 * 1024;
  const MAX_COOKIE_IMPORT_COUNT: usize = 100_000;
  const MIN_EXPORT_PASSWORD_LEN: usize = 12;
  ...
  /// store migration code will upgrade this forward when Chromium first
  ...
  /// Read cookies from a Chromium profile.
  ...
  /// Write cookies to a Chromium profile.
  ///
  ...
  /// the os_crypt key derivation between Chromium's runtime and an external
  ...
  /// UI-facing cookie index. Cookie values remain in the backend and are only
  /// materialized inside an authenticated encrypted export or a backend copy.
  pub fn read_cookie_index(profile_id: &str) -> Result<CookieReadResult, String> {
    let mut result = Self::read_cookies(profile_id)?;
    for domain in &mut result.domains {
      for cookie in &mut domain.cookies {
        cookie.value.clear();
      }
    }
    Ok(result)
  ...
  fn cookie_export_argon2() -> Result<Argon2<'static>, String> {
    let params = Params::new(19_456, 2, 1, Some(32))
      .map_err(|e| format!("Invalid cookie export Argon2 parameters: {e}"))?;
    Ok(Argon2::new(Algorithm::Argon2id, Version::V0x13, params))
  }

  fn validate_import_cookie(cookie: &UnifiedCookie) -> Result<(), &'static str> {
    if cookie.name.is_empty()
      || cookie.name.len() > 4_096
      || cookie.name.chars().any(char::is_control)
    {
      return Err("invalid cookie name");
    }
    if cookie.value.len() > 64 * 1024 || cookie.value.chars().any(|value| value == '\0') {
  ...
  fn encrypt_cookie_export(
    plaintext: &str,
    format: &str,
    cookie_count: usize,
    password: &str,
  ) -> Result<String, String> {
    if plaintext.len() > Self::MAX_COOKIE_IMPORT_BYTES {
      return Err("Cookie export exceeds the 50 MiB safety limit".to_string());
    }
    if password.chars().count() < Self::MIN_EXPORT_PASSWORD_LEN {
      return Err(
        serde_json::json!({
          "code": "COOKIE_EXPORT_PASSWORD_TOO_SHORT",
          "params": { "min": Self::MIN_EXPORT_PASSWORD_LEN.to_string() }
        })
        .to_string(),
      );
    }
    ...
    Self::cookie_export_argon2()?
      .hash_password_into(password.as_bytes(), &salt, &mut key_bytes)
      .map_err(|e| format!("Cookie export key derivation failed: {e}"))?;

    let key = Key::<Aes256Gcm>::from(key_bytes);
    let cipher = Aes256Gcm::new(&key);
    let nonce = Nonce::from(nonce_bytes);
    let aad = format!(
      "{};format={format};count={cookie_count}",
      Self::ENCRYPTED_EXPORT_AAD
    );
    let ciphertext = cipher
      ...
      .map_err(|e| format!("Cookie export encryption failed: {e}"))?;
    key_bytes.fill(0);

    let b64 = base64::engine::general_purpose::STANDARD;
    let envelope = EncryptedCookieExport {
      format: format.to_string(),
      version: 1,
      kdf: Self::ENCRYPTED_EXPORT_KDF.to_string(),
    ...
    let json = serde_json::to_string_pretty(&envelope)
      .map_err(|e| format!("Failed to serialize encrypted cookie export: {e}"))?;
    Ok(format!("{}\n{json}\n", Self::ENCRYPTED_EXPORT_HEADER))
  }

  fn decrypt_cookie_export(content: &str, password: Option<&str>) -> Result<String, String> {
    let password = password.filter(|value| !value.is_empty()).ok_or_else(|| {
      serde_json::json!({ "code": "COOKIE_EXPORT_PASSWORD_REQUIRED" }).to_string()
    })?;
    let json = Self::strip_export_header(content)
      .map(|rest| rest.trim_start_matches(['\r', '\n']))
      .filter(|rest| !rest.is_empty())
      .ok_or_else(|| serde_json::json!({ "code": "COOKIE_EXPORT_DECRYPT_FAILED" }).to_string())?;
    let envelope: EncryptedCookieExport = serde_json::from_str(json)
      .map_err(|_| serde_json::json!({ "code": "COOKIE_EXPORT_DECRYPT_FAILED" }).to_string())?;
    if envelope.version != 1
      || envelope.kdf != Self::ENCRYPTED_EXPORT_KDF
      || envelope.cipher != "aes-256-gcm"
      || !matches!(envelope.format.as_str(), "json" | "netscape")
    {
      return Err(serde_json::json!({ "code": "COOKIE_EXPORT_DECRYPT_FAILED" }).to_string());
    }
    ...
    let ciphertext = b64
      ...
      .map_err(|_| serde_json::json!({ "code": "COOKIE_EXPORT_DECRYPT_FAILED" }).to_string())?;
    if ciphertext.len() > Self::MAX_COOKIE_IMPORT_BYTES {
      return Err(serde_json::json!({ "code": "COOKIE_EXPORT_DECRYPT_FAILED" }).to_string());
    }
  ...
  pub async fn import_cookies(
    ...
    if content.len() > Self::MAX_COOKIE_IMPORT_BYTES {
      return Err("Cookie import exceeds the 50 MiB safety limit".to_string());
    }
    let profile_manager = ProfileManager::instance();
    let profiles_dir = profile_manager.get_profiles_dir();
    let profiles = profile_manager
    ...
    let decrypted;
    let parse_content = if Self::has_export_header(content) {
      decrypted = Self::decrypt_cookie_export(content, password)?;
      decrypted.as_str()
    } else {
      content
    };
    let (mut cookies, mut parse_errors) = Self::parse_cookies(parse_content);
    if cookies.len() > Self::MAX_COOKIE_IMPORT_COUNT {
      return Err("Cookie import exceeds the 100,000 cookie safety limit".to_string());
    }
    cookies.retain(|cookie| match Self::validate_import_cookie(cookie) {
      Ok(()) => true,
      Err(error) => {
        parse_errors.push(format!("{} / {}: {error}", cookie.domain, cookie.name));
        false
      }
    });
  ...
  /// Public API: export selected cookies in a password-encrypted bundle.
  /// Plaintext export is intentionally not exposed as a Tauri command.
  pub fn export_cookies_encrypted(
    profile_id: &str,
    format: &str,
    ...
    let cookies: Vec<UnifiedCookie> = all_cookies
      .into_iter()
      .filter(|cookie| {
        selected_cookies
          .iter()
          .any(|selected| selected.domain == cookie.domain && selected.name == cookie.name)
      })
      .collect();
    if cookies.is_empty() {
      return Err("No cookies selected for export".to_string());
    }
    let plaintext = match format {
      "json" => Self::format_json_cookies(&cookies),
      "netscape" => Self::format_netscape_cookies(&cookies),
      _ => return Err(format!("Unsupported export format: {format}")),
    };
    Self::encrypt_cookie_export(&plaintext, format, cookies.len(), password)
  }
}
...
mod tests {
  ...
  #[test]
  fn test_encrypted_cookie_export_roundtrip_and_wrong_password() {
    let plaintext = r#"[{"name":"sid","value":"secret","domain":".example.com"}]"#;
    let encrypted =
      CookieManager::encrypt_cookie_export(plaintext, "json", 1, "correct horse battery staple")
        .unwrap();
    assert!(encrypted.starts_with("COCO_COOKIE_EXPORT_V1\n"));
    assert!(!encrypted.contains("secret"));
    let decrypted =
      CookieManager::decrypt_cookie_export(&encrypted, Some("correct horse battery staple"))
        .unwrap();
    assert_eq!(decrypted, plaintext);

    let error =
      CookieManager::decrypt_cookie_export(&encrypted, Some("wrong password")).unwrap_err();
    assert!(error.contains("COOKIE_EXPORT_DECRYPT_FAILED"));
    assert!(CookieManager::encrypt_cookie_export(plaintext, "json", 1, "too-short").is_err());
  }
  ...
  fn test_write_chrome_cookies_stores_plaintext_values() {
    let tmp = std::env::temp_dir().join(format!("coco_cookie_test_{}.db", uuid::Uuid::new_v4()));
  ...
  fn test_decrypt_v10_cookie_with_real_vector() {
    let profile_dir =
      std::env::temp_dir().join(format!("coco_decrypt_vector_{}", uuid::Uuid::new_v4()));
  ...
  fn test_decrypt_with_wrong_host_returns_none_or_raw() {
    let profile_dir =
      std::env::temp_dir().join(format!("coco_decrypt_wrong_host_{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&profile_dir).unwrap();
    std::fs::write(
      profile_dir.join("os_crypt_key"),
      b"OSfgzI5GUqy/pK4ANrYugw==",
  ...
  fn test_create_empty_chrome_cookies_db_then_write() {
    let dir = std::env::temp_dir().join(format!("coco_empty_chrome_{}", uuid::Uuid::new_v4()));
