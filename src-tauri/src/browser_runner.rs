...
pub struct BrowserRunner {
  ...
  /// Temporary: fingerprint generation still lives on WayfernManager until
  /// Persona migration (Phase 3). Launch/stop go through KernelRegistry.
...
impl BrowserRunner {
  ...
  async fn launch_browser_internal(
    ...
    if profile.browser == "wayfern" {
      ...
      // Launch via KernelDriver (WayfernLegacyDriver) — do not assemble CLI flags here.
      ...
      // Plan is built for inspection/logging; live spawn still goes through the
      // same driver so all kernels share one entry point.
      let launch_request = KernelLaunchRequest {
        profile: updated_profile.clone(),
        profile_path: profile_data_path.clone(),
        url: url.clone(),
        ...
        wayfern_config: Some(wayfern_config.clone()),
        persona: updated_profile.persona.clone(),
        proxy_url: wayfern_config.proxy.clone(),
        ephemeral: profile.ephemeral,
      };
      let kernel = self
        .kernel_registry
        .require(&updated_profile.browser)
        .map_err(|e| -> Box<dyn std::error::Error + Send + Sync> { e.into() })?;
      // build_launch_plan may fail if binary missing; log plan args when available
      // without embedding secrets (legacy plan uses loopback proxy only).
      match kernel.build_launch_plan(&launch_request) {
        Ok(plan) => {
          log::debug!(
            "Kernel launch plan for {}: {} args, cdp={:?}, credentials_in_args={}",
            plan.kernel_id,
            plan.args.len(),
            plan.cdp_port,
            plan.has_proxy_credentials_in_args()
          );
          let _ = plan; // silence unused in release when debug logs stripped
        }
        Err(e) => {
          // WayfernLegacyDriver plan needs an installed binary; full launch also
          // resolves the binary — surface the error only if launch fails later.
          log::debug!("Kernel launch plan preview unavailable (will rely on full launch): {e}");
        }
      }

      let wayfern_result = kernel.launch(&app_handle, launch_request).await.map_err(
        |e| -> Box<dyn std::error::Error + Send + Sync> {
          format!("Failed to launch kernel {}: {e}", updated_profile.browser).into()
        },
      )?;

      // Get the process ID from launch result
      let process_id = wayfern_result.pid.unwrap_or(0);
