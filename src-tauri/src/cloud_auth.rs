...
pub const CLOUD_API_URL: &str = "https://api.cocobrowser.com";
pub const CLOUD_SYNC_URL: &str = "https://sync.cocobrowser.com";
...
impl CloudAuthManager {
  ...
  /// Local-first build: paid gates are open without a cloud subscription.
  /// Cloud login is optional (self-hosted sync only); product features do not
  /// depend on Coco cloud entitlements.
  pub async fn has_active_paid_subscription(&self) -> bool {
    true
  }

  /// Local-first: always true (see `has_active_paid_subscription`).
  pub fn has_active_paid_subscription_sync(&self) -> bool {
    true
  }

  /// Local API + MCP automation is free in this product.
  pub async fn can_use_browser_automation(&self) -> bool {
    true
  }

  /// Same-OS fingerprint editing is free. Kernel still refuses cross-OS in v0.1.
  pub async fn can_use_cross_os_fingerprints(&self) -> bool {
    true
  }

  /// Hosted Coco cloud backup remains off unless self-hosted sync is configured.
  ...
  /// Local rate-limit budget for automation (not enforced remotely).
  ...
  pub async fn is_fingerprint_os_allowed(&self, fingerprint_os: Option<&str>) -> bool {
    ...
    match fingerprint_os {
      None => true,
      // v0.1: only same-OS fingerprints; no paid cross-OS path.
  ...
  pub async fn request_wayfern_token(&self) -> Result<(), String> {
    ...
    let result = self
      .api_call_with_retry(|access_token| {
        ...
        // api.cocobrowser.com hangs the background fetch indefinitely,
    ...
    let token = match result {
      ...
      Err(e) => {
        ...
        // primary-device restriction (see cocobrowser-infra wayfern.service.ts).
  ...
  /// Wayfern remote tokens are disabled in the local fingerprint build.
