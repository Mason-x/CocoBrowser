...
use downloaded_browsers_registry::get_downloaded_browser_versions;
...
async fn read_profile_cookies(
  ...
  tokio::task::spawn_blocking(move || cookie_manager::CookieManager::read_cookie_index(&profile_id))
    .await
...
async fn import_cookies_from_file(
  ...
  let result = cookie_manager::CookieManager::import_cookies(
    &app_handle,
    &profile_id,
    &content,
    password.as_deref(),
...
async fn export_profile_cookies(
  profile_id: String,
  format: String,
  selected_cookies: Vec<cookie_manager::SelectedCookie>,
  password: String,
) -> Result<String, String> {
  tokio::task::spawn_blocking(move || {
    cookie_manager::CookieManager::export_cookies_encrypted(
      &profile_id,
      &format,
      &selected_cookies,
      &password,
  ...
  #[cfg(target_os = "macos")]
  {
    dirs::home_dir().map(|h| {
      h.join("Library")
        ...
        .join("local.mcpb.coco-browser.coco-browser")
    })
  }
...
async fn add_mcp_to_claude_desktop_internal(app_handle: &tauri::AppHandle) -> Result<(), String> {
  ...
  let manifest = serde_json::json!({
    ...
    "name": "coco-browser",
    "display_name": "CocoBrowser",
    "version": env!("CARGO_PKG_VERSION"),
    "description": "Control CocoBrowser profiles, proxies, and automation via MCP",
    "author": { "name": "CocoBrowser" },
...
"#
  ...
  update_claude_extensions_registry("local.mcpb.coco-browser.coco-browser", Some(manifest))?;
...
fn remove_mcp_from_claude_desktop_internal() -> Result<(), String> {
  ...
  update_claude_extensions_registry("local.mcpb.coco-browser.coco-browser", None)?;
  Ok(())
}
...
pub async fn check_vpn_validity_core(
  ...
  for attempt in 0..3 {
    ...
    match ip_utils::fetch_public_ip(Some(&local_proxy_url)).await {
      ...
      Err(error) => {
        log::warn!(
          "VPN validation attempt {} failed to fetch public IP through coco-proxy: {}",
          attempt + 1,
          error
        );
      }
...
/// Test a user-configured sync endpoint without granting the WebView arbitrary
/// network access. Only a GET to the endpoint's `/health` path is allowed.
fn normalize_sync_health_url(server_url: &str) -> Result<url::Url, String> {
  let mut url = url::Url::parse(server_url.trim())
    .map_err(|error| format!("Invalid sync server URL: {error}"))?;
  if !matches!(url.scheme(), "http" | "https") {
    return Err("Sync server URL must use http or https".into());
  }
  if !url.username().is_empty() || url.password().is_some() {
    return Err("Sync server URL must not contain credentials".into());
  }
  if url.host_str().is_none() {
    return Err("Sync server URL must include a host".into());
  }
  url.set_path("/health");
  url.set_query(None);
...
#[tauri::command]
async fn check_sync_server_health(server_url: String) -> Result<(), String> {
  let url = normalize_sync_health_url(&server_url)?;
  ...
  let response = client
    ...
    .send()
    .await
    .map_err(|error| format!("Sync server health check failed: {error}"))?;
  if response.status().is_success() {
    Ok(())
  } else {
    Err(format!(
      "Sync server health check returned HTTP {}",
      response.status()
    ))
  }
}
...
fn setup_system_tray(app: &tauri::AppHandle) -> Result<(), Box<dyn std::error::Error>> {
  ...
  let show_item = MenuItemBuilder::with_id("tray_show", "Show CocoBrowser").build(app)?;
  ...
  TrayIconBuilder::with_id("main")
    ...
    .on_tray_icon_event(|tray, event| {
      ...
      // restore via the "Show CocoBrowser" menu item.
...
pub fn run() {
  ...
  // Honor COCOBROWSER_DATA_ROOT: when set, logs go to <root>/logs instead of
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      // coco-proxy workers on disk must be orphans the current app can't track.
      ...
      tauri::async_runtime::spawn(async move {
        ...
        match geoip_downloader.check_missing_geoip_database() {
          Ok(true) => {
            log::info!(
              "GeoIP database is missing for fingerprint profiles, downloading at startup..."
            );
          ...
          Ok(false) => {
            // No fingerprint profiles or GeoIP database already available
          }
          Err(e) => {
    ...
    .invoke_handler(tauri::generate_handler![
      ...
      download_browser,
      cancel_download,
      kernel::downloader::list_kernel_manifest,
      kernel::downloader::list_installed_kernels,
      kernel::downloader::install_kernel,
      kernel::downloader::install_cloak_latest,
      kernel::cloak_license::get_cloak_license_status,
      kernel::cloak_license::set_cloak_license_key,
      kernel::cloak_license::clear_cloak_license_key,
      kernel::cloak_license::get_cloak_latest_release,
      kernel::geo_consistency::match_profile_persona_to_exit,
      delete_profile,
      clone_profile,
      ...
      geoip_downloader::get_geoip_status,
      start_api_server,
      stop_api_server,
...
mod tests {
  ...
  #[test]
  fn sync_health_url_is_strict_and_canonical() {
    let url = super::normalize_sync_health_url("https://sync.example/base?q=1#fragment").unwrap();
    assert_eq!(url.as_str(), "https://sync.example/health");
    assert!(super::normalize_sync_health_url("file:///etc/passwd").is_err());
    assert!(super::normalize_sync_health_url("https://user:pass@example.com").is_err());
    assert!(super::normalize_sync_health_url("not a url").is_err());
  }
