...
pub struct ProxyManager {
  ...
  // Track the exact active_proxies key for each profile. Pending launches use
  // unique synthetic keys instead of the globally-colliding placeholder PID 0.
  profile_active_proxy_keys: Mutex<HashMap<String, u32>>,
...
impl ProxyManager {
  pub fn new() -> Self {
    let manager = Self {
      ...
      profile_active_proxy_keys: Mutex::new(HashMap::new()),
      next_pending_key: AtomicU32::new(u32::MAX),
  ...
  // Check if a proxy is valid by routing through a temporary coco-proxy process.
  ...
  pub async fn start_proxy(
    ...
    // Start a new proxy using the coco-proxy binary with the correct CLI interface
    ...
    if let Some(proxy_settings) = proxy_settings {
      ...
      // Pass credentials through the launcher's environment, never its command
      // line. The launcher removes these variables before spawning the worker;
      // the persisted worker config is protected by the OS secret store.
      if let Some(username) = &proxy_settings.username {
        proxy_cmd = proxy_cmd.env("COCO_PROXY_USERNAME", username);
      }
      if let Some(password) = &proxy_settings.password {
        proxy_cmd = proxy_cmd.env("COCO_PROXY_PASSWORD", password);
      }
    }
    ...
    // The coco-proxy binary should start the worker and then exit
    let output = proxy_cmd
      .output()
      .await
      .map_err(|e| format!("Failed to execute coco-proxy: {e}"))?;
    ...
    // Store the proxy info. A browser PID is not known during launch; never
    // put all pending launches under key 0 because concurrent profiles would
    // overwrite one another.
    let tracking_key = if browser_pid == 0 {
      self.next_pending_key.fetch_sub(1, Ordering::Relaxed)
    } else {
      browser_pid
    };
    {
      let mut proxies = self.active_proxies.lock().unwrap();
      proxies.insert(tracking_key, proxy_info.clone());
    }
    ...
    if let Some(id) = profile_id {
      ...
      let mut keys = self.profile_active_proxy_keys.lock().unwrap();
      keys.insert(id.to_string(), tracking_key);
    }
  ...
  pub async fn stop_proxy(
    ...
    // Stop the proxy using the coco-proxy binary
    ...
    match proxy_cmd.output().await {
      ...
      Ok(_) => {}
      Err(e) => log::warn!("Failed to run coco-proxy stop: {e}"),
    }
    ...
    if let Some(id) = profile_id {
      ...
      self.profile_active_proxy_keys.lock().unwrap().remove(&id);
    }
  ...
  /// Re-key a pending proxy by profile once the browser PID is known.
  pub fn update_proxy_pid_for_profile(&self, profile_id: &str, new_pid: u32) -> Result<(), String> {
    if new_pid == 0 {
      return Err("Cannot map a proxy to browser PID 0".to_string());
    }
    let old_key = self
      .profile_active_proxy_keys
      .lock()
      ...
      .get(profile_id)
      .copied()
      .ok_or_else(|| format!("No active proxy key found for profile {profile_id}"))?;

    let mut proxies = self.active_proxies.lock().unwrap();
    let proxy_info = proxies
      .remove(&old_key)
      .ok_or_else(|| format!("No active proxy found for profile {profile_id}"))?;
    if proxies.contains_key(&new_pid) {
      // Restore the pending entry and fail closed rather than silently losing
      // an unrelated live proxy on an unexpected PID collision.
      proxies.insert(old_key, proxy_info);
      return Err(format!(
        "A proxy is already tracked for browser PID {new_pid}"
      ));
    }
    proxies.insert(new_pid, proxy_info);
    self
      ...
      .lock()
      .unwrap()
      .insert(profile_id.to_string(), new_pid);
    Ok(())
  }
  ...
  pub async fn cleanup_dead_proxies(
    ...
    // X button (bypassing Coco's stop flow) or the browser crashes, the
    // worker keeps running forever. On Windows users reported dozens of
    // coco-proxy processes accumulating this way.
    {
...
mod tests {
  ...
  // Helper function to build coco-proxy binary for testing
  async fn ensure_coco_proxy_binary() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    // Build the coco-proxy binary
    println!("Building coco-proxy binary for tests...");

    let build_status = Command::new("cargo")
      .args(["build", "--bin", "coco-proxy"])
      .current_dir(project_root.join("src-tauri"))
      .status()
      .await?;

    if !build_status.success() {
      return Err("Failed to build coco-proxy binary".into());
    }

    if !proxy_binary.exists() {
      return Err("coco-proxy binary was not created successfully".into());
    }
  ...
  // Integration test that actually builds and uses coco-proxy binary
  #[tokio::test]
  async fn test_proxy_integration_with_real_proxy() -> Result<(), Box<dyn std::error::Error>> {
    // This test requires coco-proxy binary to be available
  ...
  async fn test_cli_exits_after_proxy_start() -> Result<(), Box<dyn std::error::Error>> {
    let proxy_path = ensure_coco_proxy_binary().await?;
  ...
  #[test]
  fn test_profile_pid_remap_does_not_touch_other_pending_proxy() {
    let pm = ProxyManager::new();
    let key_a = u32::MAX;
    let key_b = u32::MAX - 1;
    pm.insert_active_proxy(key_a, make_proxy_info("px_a", 9011, Some("profile_a")));
    pm.insert_active_proxy(key_b, make_proxy_info("px_b", 9012, Some("profile_b")));
    {
      let mut keys = pm.profile_active_proxy_keys.lock().unwrap();
      keys.insert("profile_a".into(), key_a);
      keys.insert("profile_b".into(), key_b);
    }

    pm.update_proxy_pid_for_profile("profile_a", 4100).unwrap();
    pm.update_proxy_pid_for_profile("profile_b", 4200).unwrap();

    assert_eq!(pm.get_active_proxy(4100).unwrap().id, "px_a");
    assert_eq!(pm.get_active_proxy(4200).unwrap().id, "px_b");
    assert!(pm.get_active_proxy(key_a).is_none());
    assert!(pm.get_active_proxy(key_b).is_none());
  }
