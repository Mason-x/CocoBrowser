...
use cocobrowser_lib::proxy_runner::{
  start_proxy_process_with_profile, stop_all_proxy_processes, stop_proxy_process,
};
use cocobrowser_lib::proxy_server::run_proxy_server;
use cocobrowser_lib::proxy_storage::get_proxy_config;
use std::process;
...
async fn main() {
  ...
  // RUST_LOG=debug or RUST_LOG=coco_proxy=trace still works for deep dives.
  ...
  let matches = Command::new("coco-proxy")
    .subcommand(
      Command::new("proxy")
  ...
  if let Some(proxy_matches) = matches.subcommand_matches("proxy") {
    if let Some(start_matches) = proxy_matches.subcommand_matches("start") {
      ...
      if let (Some(host), Some(port), Some(proxy_type)) = (
        ...
        let env_username = std::env::var("COCO_PROXY_USERNAME").ok();
        let env_password = std::env::var("COCO_PROXY_PASSWORD").ok();
        let username = start_matches
          .get_one::<String>("username")
          .or(env_username.as_ref());
        let password = start_matches
          .get_one::<String>("password")
          .or(env_password.as_ref());
      ...
      if let Some(id) = stop_matches.get_one::<String>("id") {
        ...
        let configs = cocobrowser_lib::proxy_storage::list_proxy_configs();
    ...
    if action == "start" {
      ...
      let config = if let Some(path) = config_path {
        ...
        match std::fs::read_to_string(path) {
          Ok(content) => match serde_json::from_str::<
            cocobrowser_lib::vpn_worker_storage::VpnWorkerConfig,
          >(&content)
          {
            Ok(config) => {
              log::info!(
        ...
        let storage_dir = cocobrowser_lib::proxy_storage::get_storage_dir();
        log::info!("Looking for VPN worker config in: {:?}", storage_dir);
        let mut attempts = 0;
        loop {
          if let Some(config) = cocobrowser_lib::vpn_worker_storage::get_vpn_worker_config(id) {
            log::info!(
              "Found VPN worker config: id={}, vpn_type={}, vpn_id={}",
              config.id,
              config.vpn_type,
      ...
      match config.vpn_type.as_str() {
        "wireguard" => {
          let wg_config = match cocobrowser_lib::vpn::parse_wireguard_config(&vpn_config_data) {
            Ok(c) => c,
            Err(e) => {
          ...
          let server =
            cocobrowser_lib::vpn::socks5_server::WireGuardSocks5Server::new(wg_config, port);
          if let Err(e) = server
