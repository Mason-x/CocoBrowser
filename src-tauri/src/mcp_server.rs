...
const INTERACTIVE_ELEMENTS_JS: &str = r#"(() => {
  ...
  window.__coco_interactive = interactive;
...
const SERVER_NAME: &str = "coco-browser";
...
impl McpServer {
  ...
  /// Local-first: management and headed automation tools are free.
  ...
  pub async fn start(&self, app_handle: AppHandle) -> Result<u16, String> {
    // Local-first: no Wayfern commercial terms gate.
    ...
    let actual_port = listener
      .local_addr()
      .map_err(|e| format!("Failed to read MCP listener address: {e}"))?
      .port();
  ...
  async fn rate_limit_middleware(req: Request<Body>, next: Next) -> Result<Response, StatusCode> {
    if req.uri().path().starts_with("/mcp")
      && !crate::local_rate_limit::MCP_RATE_LIMITER.try_acquire()
    {
  ...
  pub fn get_tools(&self) -> Vec<McpTool> {
    vec![
      ...
      McpTool {
        ...
        description: "Launch a local browser profile with an optional URL.".to_string(),
        input_schema: serde_json::json!({
          "type": "object",
          "properties": {
            "profile_id": {
      ...
      McpTool {
        ...
        description: "Stop a running local browser profile.".to_string(),
        input_schema: serde_json::json!({
          "type": "object",
          "properties": {
            "profile_id": {
      ...
      McpTool {
        ...
        description: "Launch multiple local browser profiles with an optional URL.".to_string(),
        input_schema: serde_json::json!({
          "type": "object",
          "properties": {
            "profile_ids": {
      ...
      McpTool {
        ...
        description: "Stop multiple running local browser profiles.".to_string(),
        input_schema: serde_json::json!({
          "type": "object",
          "properties": {
            "profile_ids": {
      ...
      McpTool {
        ...
        input_schema: serde_json::json!({
          ...
          "properties": {
            ...
            "persona": {
              "type": "object",
              "description": "Complete FingerprintPersona object returned by get_profile_fingerprint"
            }
          },
          "required": ["profile_id", "persona"]
        }),
      },
      McpTool {
      ...
      McpTool {
        ...
        input_schema: serde_json::json!({
          ...
          "properties": {
            ...
            "content": {
              "type": "string",
              "description": "Raw cookie file content (JSON, Netscape, or an encrypted Coco cookie bundle)"
            },
            "password": {
              "type": "string",
              "description": "Password for an encrypted Coco cookie bundle"
            }
          },
  ...
  async fn handle_initialize(
    ...
    let result = serde_json::json!({
      ...
      "instructions": "CocoBrowser MCP server. Use tools/list to discover available browser automation tools."
    });
  ...
  async fn dispatch_tool_call(
    ...
    match tool_name {
      ...
      // Browser interaction tools (guarded by local security settings).
      "navigate" => {
        Self::require_capability("Browser automation", true).await?;
      ...
      "evaluate_javascript" => {
        let allow = crate::settings_manager::SettingsManager::instance()
          .load_settings()
          .map(|s| s.allow_js_evaluate)
          .unwrap_or(false);
        Self::require_capability("JavaScript evaluate", allow).await?;
  ...
  async fn handle_run_profile(
    ...
    let headless = arguments
      ...
      .unwrap_or(false);
    if headless {
      let allowed = SettingsManager::instance()
        .load_settings()
        .map(|settings| settings.allow_headless_automation)
        .unwrap_or(false);
      Self::require_capability("Headless automation", allowed).await?;
    }
  ...
  async fn handle_batch_run_profiles(
    ...
    for profile_id in &profile_ids {
      ...
      let cdp_port = match Self::find_free_loopback_port() {
        Ok(port) => port,
        Err(e) => {
          lines.push(format!("{profile_id}: {}", e.message));
          continue;
        }
      };
  ...
  async fn handle_get_profile_fingerprint(
    ...
    let persona =
      crate::kernel::persona::ensure_persona(profile.persona.as_ref(), &profile.version).map_err(
        |e| McpError {
          code: -32000,
          message: format!("Invalid profile Persona: {e}"),
        },
      )?;
  ...
  async fn handle_update_profile_fingerprint(
    ...
    let persona_value = arguments.get("persona").cloned().ok_or_else(|| McpError {
      code: -32602,
      message: "Missing persona".to_string(),
    })?;
    let persona: crate::kernel::persona::FingerprintPersona = serde_json::from_value(persona_value)
      .map_err(|e| McpError {
        code: -32602,
    ...
    let profile = ProfileManager::instance()
      .update_profile_persona(app_handle, profile_id, persona)
      .await
      .map_err(|e| McpError {
        code: -32000,
        message: format!("Failed to update Persona: {e}"),
      })?;

    Ok(serde_json::json!({
      "content": [{
        "type": "text",
        "text": format!("Persona updated for profile '{}'", profile.name)
      }]
    }))
  }
  ...
  fn find_free_loopback_port() -> Result<u16, McpError> {
    let listener = std::net::TcpListener::bind("127.0.0.1:0").map_err(|e| McpError {
      code: -32000,
      message: format!("Failed to reserve loopback CDP port: {e}"),
    })?;
    listener
      .local_addr()
      .map(|addr| addr.port())
      .map_err(|e| McpError {
        code: -32000,
        message: format!("Failed to read loopback CDP port: {e}"),
      })
  }
    ...
    for attempt in 0..10 {
      ...
      let port = if profile.browser == "fingerprint-chromium" {
        crate::kernel::session::SessionManager::instance()
          ...
          .and_then(|process| process.cdp_port)
      } else {
        None
      };
      if let Some(p) = port {
  ...
  async fn handle_navigate(
    ...
    crate::kernel::fingerprint_chromium::validate_launch_url(url).map_err(|error| McpError {
      code: -32602,
  ...
  async fn handle_evaluate_javascript(
    ...
    // Defense in depth: even if the dispatcher is bypassed.
    let allow = crate::settings_manager::SettingsManager::instance()
      .load_settings()
      .map(|s| s.allow_js_evaluate)
      .unwrap_or(false);
    if !allow {
      return Err(McpError {
        code: -32000,
        message: "evaluate_javascript disabled (set allow_js_evaluate in settings)".to_string(),
      });
    }
  ...
  async fn handle_click_by_index(
    ...
    let js = format!(
      r#"(() => {{
        const arr = window.__coco_interactive;
...
mod tests {
  ...
  fn test_mcp_tools_count() {
    ...
    for inactive_tool in [
      ...
      assert!(!tool_names.contains(&inactive_tool));
    }
    // Browser interaction tools
