...
pub fn save_proxy_config(config: &ProxyConfig) -> Result<(), Box<dyn std::error::Error>> {
  ...
  if disk_config.upstream_url != "DIRECT"
    && !crate::secret_store::is_protected(&disk_config.upstream_url)
  {
    disk_config.upstream_url = crate::secret_store::protect_local(
      "proxy-upstream-url",
      disk_config.upstream_url.as_bytes(),
    )?;
  }
  let content = serde_json::to_string_pretty(&disk_config)?;
...
pub fn get_proxy_config(id: &str) -> Option<ProxyConfig> {
  ...
  match fs::read_to_string(&file_path) {
    Ok(content) => serde_json::from_str(&content)
      .ok()
      .and_then(unprotect_proxy_config),
    Err(_) => None,
  }
}
      ...
      if path.extension().is_some_and(|ext| ext == "json") {
        if let Ok(content) = fs::read_to_string(&path) {
          if let Ok(config) = serde_json::from_str::<ProxyConfig>(&content) {
            if let Some(config) = unprotect_proxy_config(config) {
              configs.push(config);
            }
          }
        }
...
pub fn update_proxy_config(config: &ProxyConfig) -> bool {
  ...
  save_proxy_config(config).is_ok()
}

fn unprotect_proxy_config(mut config: ProxyConfig) -> Option<ProxyConfig> {
  if crate::secret_store::is_protected(&config.upstream_url) {
    let plaintext =
      crate::secret_store::unprotect_local("proxy-upstream-url", &config.upstream_url)
        .map_err(|e| log::error!("Failed to decrypt proxy config {}: {e}", config.id))
        .ok()?;
    config.upstream_url = String::from_utf8(plaintext)
      .map_err(|e| log::error!("Proxy config {} is not valid UTF-8: {e}", config.id))
      .ok()?;
  }
  Some(config)
}
...
mod tests {
  ...
  #[test]
  fn persisted_upstream_credentials_are_not_plaintext() {
    let temp = tempfile::TempDir::new().unwrap();
    let _guard = crate::app_dirs::set_test_data_dir(temp.path().to_path_buf());
    let config = ProxyConfig::new(
      "protected-proxy".into(),
      "http://alice:very-secret@example.test:8080".into(),
      Some(19090),
    );
    save_proxy_config(&config).unwrap();

    let raw = fs::read_to_string(get_storage_dir().join("protected-proxy.json")).unwrap();
    assert!(!raw.contains("alice"));
    assert!(!raw.contains("very-secret"));
    assert_eq!(
      get_proxy_config("protected-proxy").unwrap().upstream_url,
      config.upstream_url
    );
  }
