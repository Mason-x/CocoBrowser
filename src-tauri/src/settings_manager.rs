...
pub struct AppSettings {
  ...
  /// Allow headless browser launches via API/MCP (default false — high risk).
  #[serde(default)]
  pub allow_headless_automation: bool,
...
impl SettingsManager {
  ...
  fn get_vault_password() -> String {
    env!("COCO_BROWSER_VAULT_PASSWORD").to_string()
  }
  ...
  pub async fn store_api_token(
    ...
    if cfg!(windows) {
      let protected = crate::secret_store::protect_local("api-automation-token", token.as_bytes())?;
      let mut file_data = Vec::with_capacity(6 + protected.len());
      file_data.extend_from_slice(b"DBAPI");
      file_data.push(3u8);
      file_data.extend_from_slice(protected.as_bytes());
      std::fs::write(&token_file, file_data)?;
      crate::app_dirs::restrict_to_owner(&token_file);
      return Ok(());
    }
  ...
  pub async fn get_api_token(
    ...
    if version == 3 {
      let protected = std::str::from_utf8(&file_data[6..])?;
      let plaintext = crate::secret_store::unprotect_local("api-automation-token", protected)?;
      return Ok(String::from_utf8(plaintext).ok());
    }
  ...
  pub async fn store_mcp_token(
    ...
    if cfg!(windows) {
      let protected = crate::secret_store::protect_local("mcp-automation-token", token.as_bytes())?;
      let mut file_data = Vec::with_capacity(6 + protected.len());
      file_data.extend_from_slice(b"DBMCP");
      file_data.push(3u8);
      file_data.extend_from_slice(protected.as_bytes());
      std::fs::write(&token_file, file_data)?;
      crate::app_dirs::restrict_to_owner(&token_file);
  ...
  pub async fn get_mcp_token(
    ...
    if version == 3 {
      let protected = std::str::from_utf8(&file_data[6..])?;
      let plaintext = crate::secret_store::unprotect_local("mcp-automation-token", protected)?;
      return Ok(String::from_utf8(plaintext).ok());
    }
    if version != 2 {
      return Ok(None);
  ...
  pub async fn store_sync_token(
    ...
    if cfg!(windows) {
      let protected =
        crate::secret_store::protect_local("self-hosted-sync-token", token.as_bytes())?;
      let mut file_data = Vec::with_capacity(6 + protected.len());
      file_data.extend_from_slice(b"DBSYN");
      file_data.push(3u8);
      file_data.extend_from_slice(protected.as_bytes());
      std::fs::write(&token_file, file_data)?;
      crate::app_dirs::restrict_to_owner(&token_file);
  ...
  pub async fn get_sync_token(
    ...
    if version == 3 {
      let protected = std::str::from_utf8(&file_data[6..])?;
      let plaintext = crate::secret_store::unprotect_local("self-hosted-sync-token", protected)?;
      return Ok(String::from_utf8(plaintext).ok());
    }
    if version != 2 {
      return Ok(None);
