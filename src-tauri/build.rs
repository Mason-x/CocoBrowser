fn main() {
  ...
  if let Ok(vault_password) = std::env::var("COCO_BROWSER_VAULT_PASSWORD") {
    println!("cargo:rustc-env=COCO_BROWSER_VAULT_PASSWORD={vault_password}");
  } else {
    // Use default password if environment variable is not set
    println!("cargo:rustc-env=COCO_BROWSER_VAULT_PASSWORD=cocobrowser-api-vault-password");
  }
  ...
  // This allows building coco-proxy sidecar without the other binaries present
...
fn external_binaries_exist() -> bool {
  ...
  let coco_proxy_name = if target.contains("windows") {
    format!("coco-proxy-{}.exe", target)
  } else {
    format!("coco-proxy-{}", target)
  };

  binaries_dir.join(&coco_proxy_name).exists()
}
...
/// Write `bytes` to `path` only when the contents differ.
///
/// The generated icons live in the source tree (`icons/`), and `tauri dev` watches
/// `src-tauri/` for changes. Rewriting them unconditionally bumps their mtime on
/// every build, which retriggers the watcher, which reruns this build script —
/// an endless rebuild loop that also fails with "failed to remove file
/// cocobrowser.exe (os error 5)" once a previous instance is still running.
/// Comparing first keeps the mtime stable when the SVG has not changed.
...
fn generate_tray_icons() {
  ...
  for (size, filename) in sizes {
    ...
    let encoded = pixmap.encode_png().expect("Failed to encode tray icon PNG");
    write_if_changed(&output_path, &encoded);
  }

  // Full-colour tray icons for Windows and Linux (no template conversion).
  // Both are generated from the same SVG so the tray can never fall out of sync
  // with the app mark — previously the Linux asset was committed by hand and was
  // the one file the rebrand would have missed.
  for filename in ["tray-icon-win-44.png", "tray-icon-linux-44.png"] {
    let size = 44u32;
    let mut pixmap = Pixmap::new(size, size).expect("Failed to create pixmap");
    ...
    let encoded = pixmap
      .encode_png()
      .expect("Failed to encode full-colour tray icon PNG");
    write_if_changed(&output_path, &encoded);
  }
}
